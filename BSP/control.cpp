#include "control.h"
#define left_ccd_limit 183
#define right_ccd_limit 13

#define linear_Limit 8
#define three_second_limit 30

#define linear_k 1
#define Quadratic_k 0.12
#define three_second_k 0.1
...
{   
    if(state == close)
        ...
        return three_second_k*(error-three_second_limit)*sqrt((error-three_second_limit)) 
								+ linear_k*(three_second_limit-linear_Limit);
		else
				return 0;
}
...
{
PID_struct_init(&revise_ccd_pid,POSITION_PID,0.8,0,0.015,0,0.001);
}
...
void car_state::pos_update(int ccd_pos)
{
    ...
    {
        car_pos=right;
        if(ccd_pos<(ccd_center - three_second_limit)&&ccd_pos>=(right_ccd_limit))
        {
            car_error_state = far;
        }
        else if(ccd_pos<(ccd_center - linear_Limit)&&ccd_pos>=(ccd_center - three_second_limit))
        {
    ...
    {
        car_pos=left;
        if(ccd_pos<(left_ccd_limit)&&ccd_pos>=(ccd_center + three_second_limit))
        {
            car_error_state = far;
        }
        else if(ccd_pos<(ccd_center + three_second_limit)&&ccd_pos>=(ccd_center + linear_Limit))
        {
            car_error_state = close;
        }
